...
object TypedPipe extends Serializable {
  def from[T](pipe: Pipe, fields: Fields)(implicit tManifest: Manifest[T], conv: TupleConverter[T]): TypedPipe[T] =
    TypedPipeInst[T](pipe, fields, Converter(conv))

  def from[T](mappable: TypedSource[T])(implicit flowDef: FlowDef, mode: Mode, tManifest: Manifest[T]): TypedPipe[T] =
  ...
  def from[T](iter: Iterable[T])(implicit flowDef: FlowDef, mode: Mode, tManifest: Manifest[T]): TypedPipe[T] =
    IterablePipe[T](iter, flowDef, mode)

  /** Input must be a Pipe with exactly one Field */
  def fromSingleField[T: Manifest](pipe: Pipe): TypedPipe[T] =
...
/////////////////////////////////////////////
   ...
   * performed.
   */
  def ++[U >: T: Manifest](other: TypedPipe[U]): TypedPipe[U] = other match {
   ...
   */
  def aggregate[B,C: Manifest](agg: Aggregator[T, B, C]): ValuePipe[C] =
  ...
  def asKeys[U>:T](implicit uManifest: Manifest[U], ord: Ordering[U]): Grouped[U, Unit] = map((_, ())).group
   ...
   * }
   */
  def collect[U: Manifest](fn: PartialFunction[T, U]): TypedPipe[U] =
    filter(fn.isDefinedAt(_)).map(fn)

  /** Attach a ValuePipe to each element this TypedPipe
   */
  def cross[V: Manifest](p: ValuePipe[V]) : TypedPipe[(T, V)] =
    p match {
  ...
  def distinct(implicit ord: Ordering[_ >: T]): TypedPipe[T] =
    asKeys(implicitly[Manifest[T]], ord.asInstanceOf[Ordering[T]]).sum.keys
  ...
  def either[R: Manifest](that: TypedPipe[R]): TypedPipe[Either[T, R]] =
  ...
  def eitherValues[K,V,R](that: TypedPipe[(K, R)])(
    implicit ev: T <:< (K,V),
    kManifest: Manifest[K],
    tManifest: Manifest[V],
    rManifest: Manifest[R]
  ): TypedPipe[(K, Either[V, R])] =
    mapValues { (v: V) => Left(v) } ++ (that.mapValues { (r: R) => Right(r) })

  /** Transform each element via the function f */
  def map[U: Manifest](f: T => U): TypedPipe[U] = flatMap { t => Iterator(f(t)) }
   ...
   */
  def forceToDisk: TypedPipe[T] = TypedPipe.fromSingleField(fork.toPipe(0).forceToDisk)
  ...
  def groupBy[K](g: T => K)(implicit kManifest: Manifest[K], ord: Ordering[K]): Grouped[K,T] =
    map { t => (g(t), t) }.group
   ...
   */
  def sum[U >: T: Manifest: Semigroup]: ValuePipe[U] = ComputedValue(groupAll.sum[U].values)

  /** Reasonably common shortcut for cases of associative/commutative reduction by Key
   */
  def sumByKey[K,V](implicit ev: T<:<(K,V), kManifest: Manifest[K], vManifest: Manifest[V], ord: Ordering[K], plus: Semigroup[V]): UnsortedGrouped[K, V] =
    group[K, V].sum[V]
  ...
  def unpackToPipe[U >: T](fieldNames: Fields)(implicit up: TupleUnpacker[U]): Pipe = {
    implicit val setter = up.newSetter(fieldNames)
    toPipe[U](fieldNames)
  }
  ...
  def keys[K](implicit ev : <:<[T,(K,_)], kManifest: Manifest[K]) : TypedPipe[K] =
    // avoid capturing ev in the closure:
    map { t => t.asInstanceOf[(K, _)]._1 }

  /** swap the keys with the values */
  def swap[K,V](implicit ev: <:<[T,(K,V)], kManifest: Manifest[K], vManifest: Manifest[V]): TypedPipe[(V,K)] =
    map { _.asInstanceOf[(K,V)].swap }

  /** Just keep the values, or ._2 (if this type is a Tuple2) */
  def values[V](implicit ev : <:<[T,(_,V)], vManifest: Manifest[V]) : TypedPipe[V] =
    // avoid capturing ev in the closure:
   ...
   */
  def leftCross[V: Manifest](p: ValuePipe[V]): TypedPipe[(T, Option[V])] =
    p match {
  ...
  def leftCross[V: Manifest](thatPipe: TypedPipe[V]): TypedPipe[(T, Option[V])] =
    map(((), _)).hashLeftJoin(thatPipe.groupAll).values

  /** common pattern of attaching a value and then map */
  def mapWithValue[U: Manifest, V: Manifest](value: ValuePipe[U])(f: (T, Option[U]) => V) : TypedPipe[V] =
    leftCross(value).map(t => f(t._1, t._2))

  /** common pattern of attaching a value and then flatMap */
  def flatMapWithValue[U: Manifest, V](value: ValuePipe[U])(f: (T, Option[U]) => TraversableOnce[V]) : TypedPipe[V] =
    leftCross(value).flatMap(t => f(t._1, t._2))

  /** common pattern of attaching a value and then filter */
  def filterWithValue[U: Manifest](value: ValuePipe[U])(f: (T, Option[U]) => Boolean) : TypedPipe[T] =
  ...
  def hashCogroup[K,V,W,R](smaller: HashJoinable[K,W])
    ...
    (implicit ev: TypedPipe[T] <:< TypedPipe[(K,V)], vManifest: Manifest[V], rManifest: Manifest[R]): TypedPipe[(K,R)] =
  ...
  def hashJoin[K,V,W](smaller: HashJoinable[K,W])(
    ...
    vManifest: Manifest[V],
    wManifest: Manifest[W]
  ): TypedPipe[(K,(V,W))] = hashCogroup[K,V,W,(V,W)](smaller)(Joiner.hashInner2)
  ...
  def hashLeftJoin[K,V,W](smaller: HashJoinable[K,W])(
    ...
    vManifest: Manifest[V],
    wManifest: Manifest[W]
  ): TypedPipe[(K,(V,Option[W]))] = hashCogroup[K,V,W,(V,Option[W])](smaller)(Joiner.hashLeft2)

  /** For each element, do a map-side (hash) left join to look up a value
   */
  def hashLookup[K>:T: Manifest,V: Manifest](grouped: HashJoinable[K, V]): TypedPipe[(K, Option[V])] =
    map((_, ()))
  ...
  def sketch[K,V](
    ...
    ordering: Ordering[K]
  ): Sketched[K,V] = Sketched(ev(this), reducers, delta, eps, seed)
...
final case class EmptyTypedPipe(@transient fd: FlowDef, @transient mode: Mode) extends TypedPipe[Nothing] {
  ...
  override implicit val tManifest: Manifest[Nothing] = implicitly[Manifest[Nothing]]
...
final case class IterablePipe[T](iterable: Iterable[T],
  ...
  @transient mode: Mode)(implicit override val tManifest: Manifest[T]) extends TypedPipe[T] {

  override def aggregate[B, C: Manifest](agg: Aggregator[T, B, C]): ValuePipe[C] =
    Some(iterable)
      .filterNot(_.isEmpty)
      .map(it => LiteralValue(agg(it))(fd, mode, implicitly[Manifest[C]]))
      .getOrElse(EmptyValue()(fd, mode))

  override def ++[U >: T: Manifest](other: TypedPipe[U]): TypedPipe[U] = other match {
  ...
  override def flatMap[U: Manifest](f: T => TraversableOnce[U]) =
  ...
  override def map[U: Manifest](f: T => U): TypedPipe[U] =
  ...
  override def sum[U >: T](implicit plus: Semigroup[U], uManifest: Manifest[U]): ValuePipe[U] =
    Semigroup.sumOption[U](iterable).map(LiteralValue(_)(fd, mode, uManifest))
      .getOrElse(EmptyValue()(fd, mode))

  override def sumByLocalKeys[K,V]
      (implicit ev: T <:< (K,V), sg: Semigroup[V], kManifest: Manifest[K], vManifest: Manifest[V]) =
...
final case class TypedPipeInst[T](@transient inpipe: Pipe,
  fields: Fields,
  flatMapFn: FlatMapFn[T])(implicit override val tManifest: Manifest[T]) extends TypedPipe[T] {

  import Dsl._

  // The output pipe has a single item CTuple with an object of type T in position 0
  @transient protected lazy val pipe: Pipe = toPipe(0)(tManifest, singleSetter[T])

  // Implements a cross product.  The right side should be tiny (< 100MB)
  override def cross[U: Manifest](tiny: TypedPipe[U]): TypedPipe[(T,U)] = tiny match {
  ...
  override def flatMap[U: Manifest](f: T => TraversableOnce[U]): TypedPipe[U] =
   ...
   */
  override def toPipe[U >: T: Manifest: TupleSetter](fieldNames: Fields): Pipe =
...
final case class MergedTypedPipe[T](left: TypedPipe[T], right: TypedPipe[T])
    (implicit override val tManifest: Manifest[T]) extends TypedPipe[T] {
  import Dsl._

  // Implements a cross project.  The right side should be tiny
  def cross[U: Manifest](tiny : TypedPipe[U]): TypedPipe[(T,U)] = tiny match {
    case EmptyTypedPipe(fd, m) => EmptyTypedPipe(fd, m)
  ...
  def flatMap[U: Manifest](f: T => TraversableOnce[U]): TypedPipe[U] =
  ...
  override def sumByLocalKeys[K,V](
    ...
    kManifest: Manifest[K],
    vManifest: Manifest[V]
  ): TypedPipe[(K, V)] = MergedTypedPipe(left.sumByLocalKeys, right.sumByLocalKeys)

  override def map[U: Manifest](f: T => U): TypedPipe[U] = MergedTypedPipe(left.map(f), right.map(f))
  ...
  override def hashCogroup[K,V,W,R](smaller: HashJoinable[K,W])(joiner: (K, V, Iterable[W]) => Iterator[R])(
    implicit ev: TypedPipe[T] <:< TypedPipe[(K,V)],
    kManifest: Manifest[K],
    vManifest: Manifest[V],
    wManifest: Manifest[W],
    rManifest: Manifest[R]
  ): TypedPipe[(K,R)] = MergedTypedPipe(left.hashCogroup(smaller)(joiner), right.hashCogroup(smaller)(joiner))
}

class TuplePipeJoinEnrichment[K: Manifest: Ordering, V: Manifest](pipe: TypedPipe[(K, V)]) {
  def join[W: Manifest](smaller : TypedPipe[(K, W)], reducers: Int = -1) : CoGrouped[K, (V, W)] = pipe.group.withReducers(reducers).join(smaller.group)
  def leftJoin[W: Manifest](smaller : TypedPipe[(K, W)], reducers: Int = -1) : CoGrouped[K, (V, Option[W])] = pipe.group.withReducers(reducers).leftJoin(smaller.group)
  def rightJoin[W: Manifest](smaller : TypedPipe[(K, W)], reducers: Int = -1) : CoGrouped[K, (Option[V], W)] = pipe.group.withReducers(reducers).rightJoin(smaller.group)
  def outerJoin[W: Manifest](smaller : TypedPipe[(K, W)], reducers: Int = -1) : CoGrouped[K, (Option[V], Option[W])] = pipe.group.withReducers(reducers).outerJoin(smaller.group)
}

class MappablePipeJoinEnrichment[T](pipe: TypedPipe[T]) {
  def joinBy[K: Manifest: Ordering, U](smaller : TypedPipe[U])(g : (T => K), h : (U => K), reducers: Int = -1): CoGrouped[K, (T, U)] = pipe.groupBy(g).withReducers(reducers).join(smaller.groupBy(h))
  def leftJoinBy[K: Manifest: Ordering, U](smaller : TypedPipe[U])(g : (T => K), h : (U => K), reducers: Int = -1): CoGrouped[K, (T, Option[U])] = pipe.groupBy(g).withReducers(reducers).leftJoin(smaller.groupBy(h))
  def rightJoinBy[K: Manifest: Ordering, U](smaller : TypedPipe[U])(g : (T => K), h : (U => K), reducers: Int = -1): CoGrouped[K, (Option[T], U)] = pipe.groupBy(g).withReducers(reducers).rightJoin(smaller.groupBy(h))
  def outerJoinBy[K: Manifest: Ordering, U](smaller : TypedPipe[U])(g : (T => K), h : (U => K), reducers: Int = -1): CoGrouped[K, (Option[T], Option[U])] = pipe.groupBy(g).withReducers(reducers).outerJoin(smaller.groupBy(h))
}

object Syntax {
  implicit def joinOnTuplePipe[K: Manifest: Ordering, V: Manifest](p: TypedPipe[(K, V)]): TuplePipeJoinEnrichment[K, V] = new TuplePipeJoinEnrichment(p)
