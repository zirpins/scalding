...
abstract class VersionedCassandraStoreInitializer[KeyT, ValT](
  ...
  override def toString(): String = {
    s"""[CassandraCfVersionedStore(enpoint:${getStoreSession.getSession.getState().getConnectedHosts().toArray().toList.map(_.toString).mkString("|")} 
			,cluster:${getStoreSession.getCluster.getClusterName()}
			,keyspace:${getStoreSession.getKeyspacename}
			,CF-basename:${getCFBaseName})"""
  }
